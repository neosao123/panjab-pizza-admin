@extends('template.master', ['pageTitle' => 'Signature Pizza Update'])
@push('styles')
  ...
  <style>
    ...
    .form-check-container {
      display: flex;
      flex-wrap: wrap;
      /* Ensures that checkboxes wrap when space runs out */
      gap: 15px;
      /* Space between the checkboxes */
    }

    .form-check {
      display: inline-block;
      /* Ensure each checkbox and label are displayed inline */
      margin-bottom: 10px;
      /* Vertical margin between rows of checkboxes */
    }
    ...
    .form-check-label {
      display: inline-block;
      /* Ensures the label aligns with the checkbox */
      white-space: nowrap;
      /* Prevents text wrapping if the label is long */
    }
  ...
  <div class="page-breadcrumb">
    <div class="row">
      <div class="col-5 align-self-center">
        <h4 class="page-title">Signature Pizza</h4>
        <div class="d-flex align-items-center">
          <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
              <li class="breadcrumb-item"><a href="{{ url('/dashboard') }}">Home</a></li>
              <li class="breadcrumb-item"><a href="{{ url('/signature-pizza/list') }}">Signature Pizza</a></li>
      ...
      <div class="col-7 align-self-center ">
        <a href="{{ url('/signature-pizza/list') }}" class="btn btn-outline-secondary btn-sm float-right">Back</a>
      </div>
      <?php } ?>
    </div>
  </div>
    ...
    <div class="container-fluid">
      <div class="card">
        ...
        <div class="card-body">
          ...
          @endif
          <form action="{{ url('/signature-pizza/update') }}" id="inFrom" method="post" enctype="multipart/form-data" data-parsley-validate="">
            @csrf
            <div class="row">
              ...
              <div class="col-md-6 form-group">
                ...
                <input type="text" id="name" name="name" class="form-control" required value="{{ $queryresult->pizza_name }}" data-parsley-required-message="Name is required" maxlength='150'
              ...
              <div class="col-md-3 form-group">
                ...
                <input type="text" id="subtitle" name="subtitle" class="form-control" value="{{ $queryresult->pizza_subtitle }}" maxlength='150' data-parsley-minlength="3"
              ...
              <div class="col-md-12 form-group">
                ...
                <textarea id="description" name="description" class="form-control summernote" rows="4" data-parsley-minlength="2" data-parsley-minlength-message="You need to enter at least 2 characters"
              ...
              @foreach ($pizzaPrices as $item)
                ...
                <div class="col-sm-6 col-md-4 col-lg-3 form-group">
                  ...
                  <!-- Price input -->
                  <input type="number" name="pizzaPrice[{{ $cnt }}][price]" step="0.01" min="0" max="9999" class="form-control" value="{{ $price }}" required>

                  <!-- Radio button for default selection -->
                  <div class="custom-control custom-radio mt-2 d-none">
                    <input class="custom-control-input" type="radio" id="isDefault_{{ $item->shortcode }}" name="isDefault" value="{{ $item->shortcode }}" {{ $isDefault ? 'checked' : '' }}>
                  ...
                  <select class="select2 form-control" id="cheese" name="cheese" style="width: 100%;" required data-parsley-required-message="Cheese is required">
                    ...
                    @if (!empty($cheese))
                      @foreach ($cheese as $item)
                        <option value="{{ json_encode(['code' => $item->code, 'title' => $item->cheese]) }}" {{ (json_decode($queryresult->cheese)->code ?? '') == $item->code ? 'selected' : '' }}>
                          {{ $item->cheese }}
                      @endforeach
                    @endif
                  </select>
                  ...
                  <select class="select2 form-control" id="crust" name="crust" style="width: 100%;" required data-parsley-required-message="Crust is required">
                    ...
                    @if (!empty($crust))
                      @foreach ($crust as $item)
                        <option value="{{ json_encode(['code' => $item->code, 'title' => $item->crust]) }}" {{ (json_decode($queryresult->crust)->code ?? '') == $item->code ? 'selected' : '' }}>
                          {{ $item->crust }}
                        </option>
                      @endforeach
                    @endif
                  ...
                  <select class="select2 form-control" id="spices" name="spices" style="width: 100%;" required data-parsley-required-message="Spices is required">
                    ...
                    @if (!empty($spices))
                      @foreach ($spices as $item)
                        <option value="{{ json_encode(['code' => $item->code, 'title' => $item->spicy]) }}" {{ (json_decode($queryresult->spices)->code ?? '') == $item->code ? 'selected' : '' }}>
                          {{ $item->spicy }}
                        </option>
                      @endforeach
                    @endif
                  ...
                  <select class="select2 form-control" id="sauce" name="sauce" style="width: 100%;" required data-parsley-required-message="Sauce is required">
                    ...
                    @if (!empty($sauce))
                      @foreach ($sauce as $item)
                        <option value="{{ json_encode(['code' => $item->code, 'title' => $item->sauce]) }}" {{ (json_decode($queryresult->sauce)->code ?? '') == $item->code ? 'selected' : '' }}>
                          {{ $item->sauce }}
                        </option>
                      @endforeach
                    @endif
                  ...
                  <select class="select2 form-control" id="cook" name="cook" style="width: 100%;" required data-parsley-required-message="Cook is required">
                    ...
                    @if (!empty($cook))
                      @foreach ($cook as $item)
                        <option value="{{ json_encode(['code' => $item->code, 'title' => $item->cook]) }}" {{ (json_decode($queryresult->cook)->code ?? '') == $item->code ? 'selected' : '' }}>
                          {{ $item->cook }}
                        </option>
                      @endforeach
                    @endif
                ...
                <div class="form-check-container">
                  @if (!empty($toppingFree))
                    @php
                      ...
                      $selectedFreeToppings = collect(json_decode($queryresult->topping_as_free ?? '[]'))
                          ->pluck('code')
                          ->all();
                    @endphp
                    ...
                    @foreach ($toppingFree as $topping)
                      <div class="custom-control custom-checkbox">
                        <input type="checkbox" class="custom-control-input" id="free_topping_{{ $topping->code }}" name="free_topping[]"
                          value="{{ json_encode(['code' => $topping->code, 'title' => $topping->toppingsName]) }}" {{ in_array($topping->code, $selectedFreeToppings) ? 'checked' : '' }}
                ...
                <div class="form-check-container">
                  @if (!empty($toppingAsOne))
                    @php
                      ...
                      $selectedToppingsOne = collect(json_decode($queryresult->topping_as_1 ?? '[]'))
                          ->pluck('code')
                          ->all();
                    @endphp
              ...
              <div class="col-md-12 form-group">
                ...
                <div class="form-check-container">
                  @if (!empty($toppingAsTwo))
                    @php
                      // Decode the JSON data for selected toppings one and get only the codes
                      $selectedToppingsTwo = collect(json_decode($queryresult->topping_as_2 ?? '[]'))
                          ->pluck('code')
                          ->all();
                    @endphp
              ...
              <div class="col-md-3 form-group">
                <label class="form-label" for="form-wizard-progress-wizard-signaturepizzaimage">Signature Pizza
                  image: </label>
                <input type="file" id="file" class="form-control " name="signaturepizzaimage" accept=".jpg, .jpeg, .png">

              </div>

              @if (!empty($queryresult->pizza_image))
                <div class="col-md-3 form-group mb-3" id="eImage">
                  <img class="img-thumbnail mb-2" width="100" height="100" id="showProfileImg" src="{{ url('uploads/signature-pizza/' . $queryresult->pizza_image) . '?v=' . time() }}"
                    data-src="">
                  <a class="btn btn-danger text-white" onclick="deleteImage('{{ $queryresult->code }}','{{ $queryresult->pizza_image }}');"><i class="fa fa-trash"></i></a>
                </div>
              @endif
...
@push('scripts')
  ...
  <script type="text/javascript" src="{{ asset('theme/init_site/signature-pizza/edit.js?v=' . time()) }}"></script>
