...
class CommonController extends Controller
{
    ...
    public function homePagePizzas(Request $r)
    {
        try {
            ...
            $offerCards = DB::table('picture')
                ->leftJoin('specialoffer', function ($join) {
                    $join->on('picture.product_id', '=', 'specialoffer.code')
                        ->where('picture.pizza_type', 'special_offers');
                })
                ->leftJoin('signaturepizza', function ($join) {
                    $join->on('picture.product_id', '=', 'signaturepizza.code')
                        ->where('picture.pizza_type', 'signature_pizzas');
                })
                ->leftJoin('pizzas', function ($join) {
                    $join->on('picture.product_id', '=', 'pizzas.code')
                        ->where('picture.pizza_type', 'other_pizzas');
                })
                ->leftJoin('sidemaster', function ($join) {
                    $join->on('picture.product_id', '=', 'sidemaster.code')
                        ->where('picture.pizza_type', 'sides');
                })
                ->where('picture.isActive', 1)
                ->whereNotNull('picture.image')
                ->select(
                    'picture.*',
                    'specialoffer.name as special_name',
                    'signaturepizza.pizza_name as signature_name',
                    'pizzas.pizza_name as other_name',
                    'sidemaster.sidename as side_name'
                )
                ->get();

            $data['offerCards'] = $offerCards->map(function ($item) {
                $pizzaName = '';
                if ($item->pizza_type == 'special_offers') {
                    $pizzaName = $item->special_name;
                } elseif ($item->pizza_type == 'signature_pizzas') {
                    $pizzaName = $item->signature_name;
                } elseif ($item->pizza_type == 'other_pizzas') {
                    $pizzaName = $item->other_name;
                } elseif ($item->pizza_type == 'sides') {
                    $pizzaName = $item->side_name;
                }
                return [
                    'title' => $item->title,
