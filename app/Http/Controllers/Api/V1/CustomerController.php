...
class CustomerController extends Controller
{
    ...
    public function customer_reset_password(Request $r)
    {
        try {
            ...
            if ($customer) {
                ...
                //$sendLink = url('/api/v3/customer/verify?token=' . $token);
                $details = [
                    'title' => 'Mail from Mr singh Pizza',
                    'otp' => $otp,
                ];
                \Mail::to($r->email)->send(new \App\Mail\ResetMailOtp($details));
                $customer->resetToken = $token;
                $customer->otp = $otp;
                $customer->save();
                return response()->json(['message' => 'OTP is sent to your email address', "data" => $token], 200);
            } else {
                return response()->json(['message' => 'No users were found with the email address provided!'], 400);
            }
        } catch (\Exception $ex) {
            return response()->json(['message' => $ex->getMessage()], 400);
        }
    ...
    public function update_customer_password(Request $r)
    {
        try {
            ...
            return response()->json(["message" => "OTP used for reset password is wrong . Please Forgot Password Again to Continue."], 400);
        } catch (\Exception $ex) {
            return response()->json(['message' => $ex->getMessage()], 400);
        }
    }
