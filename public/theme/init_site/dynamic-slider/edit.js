...
function addRow() {
    ...
    var html = `
        <tr id="row${rowCount}" class="tblrows" data-type="">
            <input type="hidden" id="addr_code${rowCount}" name="addr_code[]" value="##NA" />
            <td>
            ...
            <td>
                <button type="button" class="btn btn-sm btn-outline-danger del-button" data-value="##NA" data-row-id="row${rowCount}"><i class="fa fa-trash"></i></button>
            </td>
        </tr>
    `;
    tblBody.append(html);
...
$(document).on("click", "button.del-button", function (e) {
    ...
    if (length == 1) {
        ...
        if (value === "##NA") {
            ...
            $.ajax({
                url: baseUrl + "/dynamic-sliders/deleteLineentries/" + value, // Added a "/" before the value
                type: "GET", // Changed "get" to "GET" for consistency
            ...
            reader.onload = function (event) {
                ...
                img.onload = function () {
                    if (img.width >= 1280 && img.height >= 480) {
                        ...
                        toastr.error('Invalid image dimensions. The image size is smaller than 1280 x 480 pixels..', 'Background Image', {
                            "progressBar": false
                        });
                        $(document).find('#background_image').val(null);
                        $('#showImage').attr('src', '');
                        $('#showImage').addClass('d-none');
                        deleteImage(image_code, image_value);
                    }
                };
                img.onerror = function () {
                    toastr.error('Error loading image.', 'Background Image', {
                        "progressBar": false
                    });
                    deleteImage(image_code, image_value);
                    $(document).find('#background_image').val(null);
            ...
            reader.onload = function (event) {
                ...
                img.onload = function () {
                    if (img.width >= 720 && img.height >= 648) {
                        ...
                        toastr.error('Invalid image dimensions. The image size is smaller than 720 X 648 pixels..', 'Medium Image', {
                            "progressBar": false
                        });
                        $(document).find('#background_image_md').val(null);
            ...
            reader.onload = function (event) {
                ...
                img.onload = function () {
                    if (img.width >= 480 && img.height >= 432) {
                        ...
                        toastr.error('Invalid image dimensions. The image size is smaller than 480 X 432 pixels..', 'Small Image', {
                            "progressBar": false
                        });
                        $(document).find('#background_image_sm').val(null);
...
function deleteImage(code, value, element, btnId, imgId, size) {
    $.ajax({
        url: baseUrl + '/dynamic-sliders/deleteImage',
        method: "get",
        data: {
            'value': value,
            'code': code,
        ...
        success: function (data) {
            if (data = "true") {
                $(document).find(`#${element}`).val(null);
                $(`#${imgId}`).attr('src', '');
            ...
            alert('Failed to process your request and delete the image.');
        }
    });
}